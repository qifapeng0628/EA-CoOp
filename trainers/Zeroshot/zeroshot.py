...
from open_clip.src.open_clip import create_model_from_pretrained,create_model_and_transforms
from open_clip.src.open_clip.constants import OPENAI_DATASET_MEAN, OPENAI_DATASET_STD
...
@TRAINER_REGISTRY.register()
class ZeroshotUnimedCLIP(TrainerX):
    def build_model(self):
        cfg = self.cfg
        classnames = self.dm.dataset.classnames

        # print(f"Loading UnimedCLIP (backbone: {cfg.MODEL.BACKBONE.NAME})")
        # clip_model, preprocess = create_model_from_pretrained('hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224')
        # tokenizer = get_tokenizer('hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224')
        # clip_model.eval().to(self.device)
        model_name = 'ViT-B-16-quickgelu' # available pretrained weights ['ViT-L-14-336-quickgelu', 'ViT-B-16-quickgelu']
        if not cfg.TRAINER.UNIMEDCOOP.WEIGHTS:
            raise ValueError("必须在配置文件中指定 TRAINER.UNIMEDCOOP.WEIGHTS 路径")

        pretrained_weights = cfg.TRAINER.UNIMEDCOOP.WEIGHTS
        print(f"正在加载 UnimedCLIP: {model_name}，权重: {pretrained_weights}")
        text_encoder_name = "microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract"
        mean, std = OPENAI_DATASET_MEAN, OPENAI_DATASET_STD
        #text_cfg_dict = {'hf_model_name': text_encoder_name,'hf_model_pretrained': False}
        text_cfg_dict = {'hf_model_name': text_encoder_name,}
        clip_model, _, preprocess = create_model_and_transforms(model_name,pretrained_weights,precision=cfg.TRAINER.UNIMEDCOOP.PREC, device='cpu',force_quick_gelu=True,image_mean=mean,image_std=std,force_custom_text=True,text_cfg=text_cfg_dict)
        tokenizer = get_tokenizer("hf-hub:microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract",context_length=256)
...
@TRAINER_REGISTRY.register()
class ZeroshotUnimedCLIP2(ZeroshotUnimedCLIP):
    """Prompt ensembling."""
