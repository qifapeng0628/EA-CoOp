...
def load_checkpoint(model, checkpoint_path, strict=True):
    ...
    # --- 在这里添加您的代码 ---
    # 检查是否存在 'text_encoder.' 前缀 (来自 UnimedCLIP/BiomedCLIP)
    # 并且模型期望的前缀是 'text.' (来自 CustomTextCLIP)
    if any(k.startswith('text_encoder.') for k in state_dict.keys()) and hasattr(model, 'text'):
        print("检测到 'text_encoder.' 前缀，正在重命名为 'text.' ...")
        new_state_dict = {}
        for k, v in state_dict.items():
            if k.startswith('text_encoder.'):
                # 将 'text_encoder.layer.0.weight' 替换为 'text.layer.0.weight'
                new_k = 'text.' + k[len('text_encoder.'):]
                new_state_dict[new_k] = v
            else:
                new_state_dict[k] = v # 保持 'visual.' 等键不变
...
def create_model(
    ...
    if has_hf_hub_prefix:
        ...
        # checkpoint_path = download_pretrained_from_hf(model_id,revision='9f341de' ,cache_dir=cache_dir)
