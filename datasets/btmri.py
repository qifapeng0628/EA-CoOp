...
class BTMRI(DatasetBase):
    ...
    def read_and_split_data(image_dir, p_trn=0.5, p_val=0.2, ignored=[], new_cnames=None):
        ...
        categories = listdir_nohidden(image_dir) # # listdir_nohidden 是一个工具函数，获取 image_dir 下所有子目录（即类别名）。
        categories = [c for c in categories if c not in ignored] # # 过滤掉 "ignored" 列表中的目录
        categories.sort()
